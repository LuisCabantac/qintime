...
export default function UserCard({ attendee }: { attendee: IAttendee }) {
  ...
  return (
    <>
      <li className="group flex items-center gap-2 border-b border-[#dee2e6] px-4 py-2 hover:bg-[#f1f3f5] md:px-2">
        <div
        ...
        >
          <div>
            ...
            <p className="text-sm">{attendee.section}</p>
          </div>
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm font-medium">In time</p>
              <p className="text-sm">
                {attendee.inTime ? attendee.inTime : "N/A"}
              </p>
            </div>
            <div>
              <p className="text-sm font-medium">Out time</p>
              <p className="text-sm">
                {attendee.outTime ? attendee.outTime : "N/A"}
              </p>
            </div>
          </div>
        </div>
        ...
        <li className="fixed left-0 top-0 z-10 flex h-full w-full items-center justify-center bg-[#f1f3f5]">
          <div className="grid gap-1">
            <QRCodeSVG
              ...
              bgColor="#f1f3f5"
              size={180}
              value={`https://qintime.vercel.app/verify/${attendee.id}`}
            />
            <div className="flex flex-col items-center justify-center">
              <h2 className="font-medium">{attendee.name}</h2>
